...
def evaluate(model, data_loader, device, criterion, num_classes, k, base_dir='runs'):
    ...
    with torch.no_grad():  # Disable gradient computation for validation
        for inputs, labels in data_loader:
            ...
            all_probs = torch.cat((all_probs, torch.softmax(logits, dim=1)))  # Softmax for probabilities
...
if __name__ == "__main__":
    # Data preparation
    data_dir = r"C:\Users\ACER\OneDrive\Desktop\PAOLO\MSU-IIT\BS COM ENG (1st Sem_2024-2025)\COE190\Yolo\vit_rice_classification\archive\extra_resized_raw_images\extra_resized_raw_images"
    test_loader = get_test_loader(batch_size=32, test_dir=data_dir, input_size=(224, 224))

    # Testing
    test_model(test_loader, num_classes=14, model_path='runs/train_1/best_model.pt')
